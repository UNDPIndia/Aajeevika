...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        ->select($productname, 'price', 'id', 'product_id_d','image_1', 'template_id')
        ->get();
        ...
        foreach ($shgproduct as $value) {
            ...
            $product = ProductMaster::where(['user_id' => $value->id, 'is_active' => 1, 'is_draft' => 0])
                    ->select('id', 'product_id_d','image_1', 'price', 'template_id', 'localname_kn', 'localname_en')->limit(3)->get();
        ...
        $shguser = User::wherein('role_id', [ 2, 3 ])->select('title', 'id','organization_name')->where('isActive', 1)->take(5)->get();

        $shgArr = [];

        foreach ($shguser as $key => $value) {
            $productCheck = ProductMaster::where(['is_active' => 1, 'is_draft' => 0, 'user_id' => $value->id])->select($productname, 'product_id_d','price', 'id', 'image_1')->take(3)->get();
        ...
        $recentlyproduct = ProductMaster::where(['is_active' => 1, 'is_draft' => 0])->orderBy('id', 'desc')->select($productname, 'product_id_d','price', 'id', 'image_1', 'template_id','user_id')->with('template:id,'.$templatename,'user:id,organization_name')->take(5)->get();
        ...
        if ($lengthshg > 0) {
            ...
            if ($shguser[$position]->id) {
                ...
                $shguser = User::wherein('role_id', [ 2, 3 ])->select('title', 'id','organization_name')
                ...
                foreach ($shguser as $key => $value) {
                    ...
                    if (count($productCheck) > 0) {
                        ...
                        $shg2ndArr[$key]['organization_name'] = $value->organization_name;
        ...
        //popular seller
        $popularRating = DB::select("SELECT user_id, AVG(rating) as rating FROM `ratings` where `type`='buyer' GROUP BY user_id ORDER BY rating");
        $popularRatArr = [];
        foreach ($popularRating as $value) {
            $popularRatArr[] = $value->user_id;
        }
        $sellerDatas = User::whereIn('id', $popularRatArr)->select('id','name','organization_name', 'email', 'mobile', 'profileImage')->get(10);

        $popularSeller = [];
        foreach($sellerDatas as $values) {
            $ratingCount = Rating::where('user_id', $values->id)->count();
            $ratingStarAvg = DB::table('ratings')->where('user_id', $values->id)->avg('rating');
            $rating = [
        ...
        return view('frontend.home', ['popularSeller'=>$popularSeller, 'language'=>$language,'banner'=>$banner, 'midCategory'=> $midCategory,'response'=>$response, 'popup'=>$popup, 'shgids'=>$shgids]);
    }
    public function popularSeller(Request $request)
    {
        # code...
        try{
            //popular seller
        ...
        foreach($sellerDatas as $values) {
            ...
            return view('frontend.popular_seller',['popularSeller'=>$popularSeller]);
        }catch(\EXCEPTION $e){
            return $e->getMeassge();
        }
    }
    ...
    {
        $cityData = Block::where(['city_id' => $request->city_id])->get();
    ...
    public function blockAjaxx(Request $request)
    {
        $language = $request->session()->get('weblangauge');
        ...
        $cityData = DB::table('blocks')
        ->where('city_id', '=', $city_id)
        ->where('status', 0)
        ->select($name, 'id', 'city_id')
        ->get();
        //$output = '<option value="">ब्लॉक चुनें</option>';
        $output = "";
        foreach($cityData as $item)
        {
            //$output.='<option value="'.$item->id.'">'.$item->name_kn.'</option>';
            $output .= '<option value="'.$item->id.'"> '.$item->name.'</option>';
        }
            print_r($output);
    }
    ...
    public function singleCategory(Request $request, $id)
    {
        ...
        foreach ($categoryDetail as $key => $category) {
            ...
            foreach ($subcategory as $key1 => $sub) {
                if (count($request->shgartisanId) > 0) {
                    $allUserId = $request->shgartisanId;
                    $productdata = ProductMaster::whereIn('user_id', $allUserId)->with('template')->where(['is_active' => 1, 'is_draft' => 0, 'categoryId' => $category->id, 'subcategoryId' => $sub->id])->select($productname, 'subcategoryId', 'price', 'id', 'image_1', 'template_id','user_id')->with('user:id,name')->take(4)->orderBy('id', 'desc')->get();
                } else {
                    $productdata = ProductMaster::with('template')->where(['is_active' => 1, 'is_draft' => 0, 'categoryId' => $category->id, 'subcategoryId' => $sub->id])->select($productname, 'subcategoryId', 'price', 'id', 'image_1', 'template_id','user_id')->with('user:id,name')->take(4)->orderBy('id', 'desc')->get();
                }
    ...
    public function subCategory(Request $request, $id, $subcatid)
    {
        ...
        if ($request->has('show')) {
            ...
            $productdata = ProductMaster::where(['user_id'=>$shg_id, 'is_active' => 1, 'is_draft' => 0, 'subcategoryId' => $subcategoryId])->select($productname, 'price', 'id', 'image_1', 'template_id','user_id')->with('template:id,'.$templatename,'user:id,name')->get();
        ...
        if ($request->has('id')) {
            ...
            $productdata = ProductMaster::where(['user_id'=>$user_id, 'is_active' => 1, 'is_draft' => 0, 'subcategoryId' => $subcategoryId])->select($productname, 'price', 'id', 'image_1', 'template_id','user_id')->with('template:id,'.$templatename,'user:id,name')->orderBy('id', 'desc')->get();
        } else {
            $productdata = ProductMaster::where(['is_active' => 1, 'is_draft' => 0, 'subcategoryId' => $subcategoryId])->select($productname, 'price', 'id', 'image_1', 'template_id','user_id')->with('template:id,'.$templatename,'user:id,name')->orderBy('id', 'desc')->get();
        }
    ...
    public function viewproduct(Request $request, $id)
    {
        ...
        $productdetail = ProductMaster::with('category','subcategory')->where(['id'=>$id,'is_active'=>1])->select('id','product_id_d', 'price_unit','price', 'user_id as artisanid', 'material_id', 'categoryId', 'subcategoryId', 'qty', 'length', 'width', 'height', 'vol', 'weight', 'length_unit', 'width_unit', 'height_unit', 'weight_unit', 'vol_unit', 'image_1', 'image_2', 'image_3', 'image_4', 'image_5', 'template_id', 'video_url', $productname, $descriptionname)->first();
        ...
        if ($productdetail) {
            ...
            // Product Certificate
            $certificateData = ProductCertification::where('product_id',$id)->first();
            //echo "<pre>"; print_r($certificateData); die;
            if($certificateData){
                $productdetail['certificate_data'] = $certificateData; 
            }else{
                $productdetail['certificate_data'] = NULL;
            }
        } else {
            abort(404);
        }
    ...
    public function getshgartisanhome(Request $request, $id)
    {
        ...
        $ratingCount = Rating::where('user_id', $id)->count();
        $ratingStarAvg = DB::table('ratings')->where('user_id', $id)->avg('rating');
        ...
        $data = ['user' => $userdata, 'favStatus' => $favStatus,'address' => $address,'allProduct'=>$allProduct,'reviewCount' => $ratingCount,'ratingAvgStar' => $rating_avg];
    ...
    public function recentproduct(Request $request)
    {
        ...
        $recentlyproduct = ProductMaster::where(['is_active' => 1, 'is_draft' => 0])->select($productname, 'price', 'id', 'image_1', 'template_id','user_id')->with('template:id,'.$templatename,'user:id,organization_name')->orderBy('id', 'DESC')->paginate(500);
	...
	public function updateFav(Request $request )
    {
        ...
        //echo $statusUpdate;die;
        $fav = Favorite::updateOrCreate([
            'user_id' => $userId,
            'seller_id' => $request->seller_id,
           
        ],[ 'status' => $statusUpdate]);   //0 unfollow ,1=follow

        $queryStatus    = "Successfully.";
        $statusCode     = 200;
        $status         = true;
        return  array('status' => $status , 'statusCode' =>$statusCode, 'message'=> $queryStatus);
    }
    public function viewRating(Request $request,$id)
    {
       ...
       if($userData->role_id == 9){
        ...
        $ratings = Rating::where('user_id', $id)->with('getreviews')->get();
       }
        
        return view('frontend.user.view_rating', ['ratings'=> $ratings]);
    }

	public function demoOne()
	{
		return view('frontend.demo_one');
	}
	
	public function demoTwo()
	{
		return view('frontend.demo_two');
	}
	
	public function demoThree()
	{
		return view('frontend.demo_three');
	}

    public function demoFour()
	{
		return view('frontend.demo_four');
	}
	
	public function demoFive()
	{
		return view('frontend.demo_five');
	}
	
	public function demoSix()
	{
		return view('frontend.demo_six');
	}

    public function demoSeven()
	{
		return view('frontend.demo_seven');
	}
