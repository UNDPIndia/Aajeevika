...
@section('content')
    <div class="main">
        ...
        <div class="container">
        <form method="post" action="{{ url('add-sale-product-sess') }}/{{encrypt($seller_id)}}">
        @csrf
            <div class="row">
                        ...
                        <div class="col-lg-4 col-sm-6 col-12">
                            <div class="addProductCard d-flex justify-content-between align-items-center">
                                <div class="left-box d-flex flex-wrap justify-content-start align-items-center">
                                    ...
                                    <div class="price">{{Session::get('weblangauge') == 'kn' ? 'कीमत':'Price:'}} ₹ {{ $product->price ? $product->price : '' }}/{{ $product->price_unit ? $product->price_unit : '' }}</div>
                                    <div class="price">{{Session::get('weblangauge') == 'kn' ? 'उपलब्ध':'Available:'}} {{ $product->qty_avail ? $product->qty_avail : '' }} {{ $product->price_unit ? $product->price_unit : '' }}</div>
                                    </div>
                                ...
                                <div class="qty-outer">   
                                    <div class="title w-100">{{Session::get('weblangauge') == 'kn' ? 'मात्रा:':'Qty:'}}</div>
