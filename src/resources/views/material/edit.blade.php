...
<section class="content">
      ...
      @if ($errors->any())
        ...
        @endif
          ...
          <div class="card-body">
            <form enctype="multipart/form-data" action="{{ url('admin/editMaterial') }}/{{ encrypt($id) }}" method="post">
                @csrf
                <div class="row">

                    <div class="col-sm-6 form-group">
                        <label for="">Name English *: </label>
                        <input type="text" class="form-control" name="name_en" id="" value="{{ $materialData->name_en }}">
                    </div>
                    <div class="col-sm-6 form-group">
                        <label for="">Name Hindi *: </label>
                        <input type="text" class="form-control" name="name_kn" id="" value="{{ $materialData->name_kn }}">
                    </div>
                        ...
                        <div class="col-sm-6 form-group">
                            ...
                            <select class="form-control getsubcats" name="category_id" id="">
                                ...
                                @foreach ($categoryData as $item)
                                    <option  @if( $materialData->category_id == $item->id ) selected  @endif value="{{ $item->id }}">{{ $item->name_en }}</option>
                                @endforeach 
                        ...
                        <div class="col-sm-6 form-group">
                            ...
                            <select class="form-control" name="subcategory_id" id="subcats">
                                ...
                                @foreach ($subcategoryData as $item)
                                    <option  @if( $materialData->subcategory_id == $item->id ) selected  @endif value="{{ $item->id }}">{{ $item->name_en }}</option>
                                @endforeach 
                            </select>
                        </div>
                ...
                {{-- <div class="col-sm-6 form-group">
                        ...
                        <br>
                        <img src="{{asset('/images/material')}}/{{$materialData->image}}" height="200" >
                    </div>
                   
                    <div class="col-sm-6 form-group">
                        <label for="">New Material Image: </label>
